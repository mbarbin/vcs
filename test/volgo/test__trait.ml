...
let%expect_test "unimplemented" =
  ...
  let test f = require_does_raise f in
  (* add *)
  test (fun () -> Vcs.add vcs ~repo_root ~path:(Vcs.Path_in_repo.v "foo"));
  [%expect
    {|
    ((context (Vcs.add (repo_root /path/to/repo) (path foo)))
      (error
  ...
  [%expect
    {|
    ((context (Vcs.rename_current_branch (repo_root /path/to/repo) (to_ foo)))
      (error
  ...
  [%expect
    {|
    ((context (Vcs.set_user_name (repo_root /path/to/repo) (user_name user1)))
      (error
  ...
  [%expect
    {|
    ((context
       (Vcs.set_user_email (repo_root /path/to/repo) (user_email user1@mail.com)))
      (error
  ...
  [%expect
    {|
    ((context (Vcs.ls_files (repo_root /path/to/repo) (below ./)))
      (error
  ...
  [%expect
    {|
    ((context
       (Vcs.name_status (repo_root /path/to/repo)
         (changed
           (Between (src 3e8f3b8084fe4864ab5ecf955a8cf5093e8f3b80)
  ...
  [%expect
    {|
    ((context
       (Vcs.num_status (repo_root /path/to/repo)
         (changed
           (Between (src 3e8f3b8084fe4864ab5ecf955a8cf5093e8f3b80)
             (dst 8f2865699c137a14c65ae28b83fde96b8f286569)))))
      (error
  ...
  [%expect
    {|
    ((context
       (Vcs.show_file_at_rev (repo_root /path/to/repo)
         (rev 3e8f3b8084fe4864ab5ecf955a8cf5093e8f3b80) (path foo)))
      (error
