...
let%expect_test "parse_exn" =
  ...
  print_dyn
    (Dyn.record
       [ "tags", Vcs.Refs.tags refs |> Set.to_list |> Dyn.list Vcs.Tag_name.to_dyn
       ; ( "local_branches"
         , Vcs.Refs.local_branches refs |> Set.to_list |> Dyn.list Vcs.Branch_name.to_dyn
         )
       ; ( "remote_branches"
         , Vcs.Refs.remote_branches refs
           |> Set.to_list
           |> Dyn.list Vcs.Remote_branch_name.to_dyn )
       ]);
  [%expect
    {|
  ...
  print_dyn
    (Dyn.Map
       (Vcs.Refs.to_map refs
        |> Map.to_alist
        |> List.map ~f:(fun (k, v) -> Vcs.Ref_kind.to_dyn k, Vcs.Rev.to_dyn v)));
  [%expect
    {|
    map
