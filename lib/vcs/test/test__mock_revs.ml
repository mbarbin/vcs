...
let%expect_test "mock revs" =
  ...
  (* This part of the test is a bit confusing, because we are simulating a
     ...
     generator. *)
  let rev0 = Mock_revs.next t in
  let mock_rev0 = Mock_revs.next t in
  (* If a mock revision is not known, the lookup returns [None]. *)
  print_s [%sexp (Mock_revs.of_mock t ~mock_rev:mock_rev0 : Rev.t option)];
  [%expect {| () |}];
  (* Now let's bind it to an actual revision. *)
  Mock_revs.add_exn t ~rev:rev0 ~mock_rev:mock_rev0;
  print_s [%sexp (Mock_revs.of_mock t ~mock_rev:mock_rev0 : Rev.t option)];
  [%expect {| (dd5aabd331a75b90cd61725223964e47dd5aabd3) |}];
  (* Attempting to add it again will fail. *)
  let mock_rev1 = Mock_revs.next t in
  require_does_raise [%here] (fun () -> Mock_revs.add_exn t ~rev:rev0 ~mock_rev:mock_rev1);
  [%expect
    {|
  ...
  (* Finally, as a convenience we can generate mock revision on the fly with
     [to_mock]. *)
  let rev1 = Mock_revs.next t in
  let rev2 = Mock_revs.next t in
  let rev3 = Mock_revs.next t in
  let mock_rev1 = Mock_revs.to_mock t ~rev:rev1 in
  let mock_rev2 = Mock_revs.to_mock t ~rev:rev2 in
  let mock_rev3 = Mock_revs.to_mock t ~rev:rev3 in
  print_s [%sexp (mock_rev1 : Rev.t)];
  [%expect {| a6d2cfe8358a69c9600ab905ad0435f0a6d2cfe8 |}];
  print_s [%sexp (mock_rev2 : Rev.t)];
  [%expect {| c3b38f60e7a423283dede1e7a598b378c3b38f60 |}];
  print_s [%sexp (mock_rev3 : Rev.t)];
  [%expect {| 43ba44a5d1aa1704c5c4768aac828c4343ba44a5 |}];
  (* And naturally, lookup again the same revs returns the same mock revs. *)
  require_equal [%here] (module Rev) mock_rev1 (Mock_revs.to_mock t ~rev:rev1);
  require_equal [%here] (module Rev) mock_rev2 (Mock_revs.to_mock t ~rev:rev2);
  require_equal [%here] (module Rev) mock_rev3 (Mock_revs.to_mock t ~rev:rev3);
  [%expect {||}];
  ()
;;
